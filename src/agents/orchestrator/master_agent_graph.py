...
def _update_trace_context(workflow: dict) -> None:
    """
    Update MLflow trace with workflow tags for evaluation filtering.
...
def _traceback_direct_request(request_args: dict, task_stack: list[TaskStruct]) -> list[TaskStruct]:
    ...
    if is_direct:
        ...
        if agent_name not in tool_names:
            raise Exception(
                f"[{MASTER_AGENT_NAME}] agent_name must be either {tool_names}. Got {agent_name}.", SIErrorCode.INTERFACE_MISMATCH)
...
def get_graph(LM: BaseChatModel, tools: List[StructuredTool], max_last_messages: int = 1) -> StateGraph:
    ...
    workflow_agents = [tool.name for tool in list(
        filter(lambda x: x.metadata.is_workflow & (x.metadata.expose_to_user), tools))]
    non_workflow_agents = [tool.name for tool in list(filter(lambda x: (
        not x.metadata.is_workflow) & (x.metadata.expose_to_user), tools))]
    ...
    def intent_classifier(state: MasterAgentState) -> MasterAgentState:
        ...
        workflow = state.get("workflow", {})  # Get workflow once at the start
        task_stack: List[Union[dict, TaskStruct]] = deepcopy(
            # will not change the input state when pop
        ...
        if not messages:
            ...
            ai_message = create_tool_calls_message(
                EMPTY_HTIL_ARGUMENT, human_in_the_loop.name, output_args)
            task_args = json.dumps(
                dict(text=WELCOME_PROMPT.format(tasks=task_str)))
            ...
            if isinstance(last_message, ToolMessage):
                ...
                if last_message.status == EventType.error:
                    ...
                    ai_message = create_tool_calls_message(
                        ares, human_in_the_loop.name, tool_content)
                    # return {"messages": [ai_message], "task_stack": task_stack}
                    raise Exception(
                        f"Encountered error at agent {last_message.name}!", SIErrorCode.AGENT_ENDPOINT_FAILURE)
                ...
                if last_message.name == human_in_the_loop.name:
                    ...
                    human_message = HumanMessage(
                        content=last_message.content, additional_kwargs=last_message.custom_outputs)
                    task_stack = _traceback_direct_request(
                        request_args, task_stack)
                    last_messages = _get_last_human_messages(
                        state.get("messages", []), max_last_messages)
                    ai_message = htil_handler(
                        task_stack, state, LM, tools, request_args, last_messages=last_messages)
                    workflow = state.get("workflow", {})
                    return_state = _process_tool_call_with_reject(
                        ai_message, workflow, workflow_agents, non_workflow_agents)
                    # Update trace context for evaluation filtering
                    _update_trace_context(return_state.get("workflow", workflow))
                    ...
                    if not last_message.need_resume:
                        ...
                        if task_stack:  # Tasks left
                            ...
                            last_task: TaskStruct = format_task_struct(
                                task_stack[-1])
                            # TODO: should standardise???
                            last_task_json = json.loads(
                                last_task.tool_argument.arguments)
                            ...
                            ai_message = create_tool_calls_message(
                                last_task.tool_argument.arguments, last_task.task.tool_argument.name, output_args)
                            return_msg = {"messages": [
                                ai_message], "task_stack": task_stack}
                        else:
                        ...
                        # Update trace context for evaluation (task continuation/completion)
                        _update_trace_context(return_msg.get("workflow", workflow))
                        return return_msg
                    else:
                        # Task is not complete
                        frontend_outputs = last_message.custom_outputs
                        custom_outputs = last_message.custom_outputs
                        master_agent_context = custom_outputs.get(
                        ...
                        ai_message = create_tool_calls_message(
                            tool_struct.tool_argument.arguments, tool_struct.tool_argument.name, tool_content)
                        # Update trace context for evaluation (incomplete task continuation)
                        _update_trace_context(workflow)
                        return {
                            "messages": [ai_message],
                ...
                if is_direct:
                    ...
                    for task in deepcopy(reversed(task_stack)):
                        ...
                        task_stack.pop()
                human_message = HumanMessage(
                    content=last_message.content, additional_kwargs=last_message.additional_kwargs)
                task_stack = _traceback_direct_request(
                    request_args, task_stack)
                last_messages = _get_last_human_messages(
                    state.get("messages", []), max_last_messages)
                ...
                raise Exception(
                    f"[{MASTER_AGENT_NAME}] Can't handle the type of the last message. Got {type(last_message)} with the content of:\n{last_message}", SIEErrorCode.LANGGRAPH_LOGIC)
