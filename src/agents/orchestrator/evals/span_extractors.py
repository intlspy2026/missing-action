...
def get_tone_spans(
    ...
    Returns:
        DataFrame with columns: trace_id, span_id, conversation (agent response text)
    """
    traces = TraceRetriever.get_recent_traces(
        hours=hours,
        experiment_ids=experiment_ids,
        ...
        for span in trace.data.spans:
            ...
            if agent_text:
                tone_data.append({
                    "trace_id": trace_row["trace_id"],
                    "span_id": span.span_id,
                    "trace": trace_row.get("trace"),  # Link to existing trace
                    "outputs": agent_text,  # Agent response for tone evaluation
                })
