...
b. **Fallback (no SME match)**: When a document type required by INVESTIGATION PROCESSES has no matching entry in the selected GOLD_STANDARDS section, INCLUDE it using a fallback draft: write a full instruction-style request (e.g., "A copy of _", "Fully itemised _", "Provide _"), grounded in INVESTIGATION PROCESSES and case context-specific values from INITIAL REVIEW and ADDITIONAL INFORMATION. Match the cadence, level of detail, and neutral tone of the SME exemplars in the selected section. Do NOT produce short labels or one-line summaries.
</CRITICAL_RULES>
...
3. **Derive enquiries from the claimant's incident account in INITIAL REVIEW**:
    ...
    b. From this narrative, identify major field enquiries that arise directly from the claimant's description — gaps, claims, assertions, named entities, or details needing independent verification through field investigation. Examples: a specific location described in the narrative to canvas; a person named who should be interviewed; a timeline or sequence of events needing corroboration; a detail that is vague or incomplete and needs clarification; an assertion that can be independently checked.
    c. Apply all CRITICAL RULES: Party Scope (RULE 2), External Scope Only (RULE 4), Relevance Filter (RULE 5, adapted to narrative source), and Neutral Language (RULE 6).
    d. Before including a narrative-derived enquiry, ensure:
       - It does NOT duplicate an enquiry from INVESTIGATION PROCESSES. If overlap exists, merge into the methodology-derived enquiry.
