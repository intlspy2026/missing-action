...
A concern whose substance is materially about an associated or non-direct party — even when framed as "the insured's connection to X", "overlapping contact details with X", or "the insured's association with X" — is NOT a key concern. Such observations may serve as supporting context inside another concern about a direct party (e.g., as part of a non-disclosure or staged-event concern's anchoring evidence), but must not stand as a standalone concern.
