...
**RULE 9 - CROSS-RESPONDENT MERGE (applies after RULE 8, before STYLE)**: When methodology entries target different respondent categories but relate to the same location and timeframe for the same or a related investigative purpose, merge them into a single output enquiry covering all sub-tasks. A corroborating witness enquiry at the loss location and an area canvass of that same location share a related purpose (establishing facts at that location around the incident) and must be merged regardless of different people or sub-purposes. When determining whether two enquiries share the same location and timeframe, check both the enquiry title and enquiry_detail for shared references. For example:
