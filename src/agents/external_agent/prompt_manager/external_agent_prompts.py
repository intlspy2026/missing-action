...
- **Causal Explanation**: When the narrative explicitly states a reason (Y) for a circumstance (X) that enabled the incident to occur, apply this litmus test: if Y had not occurred, would the incident still have occurred? If NO, a verification request is required — but ONLY when BOTH of these gates pass: (1) the Investigation Type Relevance Gate is satisfied, and (2) Party Scope is satisfied (the record to verify Y belongs to and is about a direct party). If Party Scope blocks Y, the Causal Explanation rule produces NO output — do NOT reinterpret Y or derive any other document from the same fragment. If the Investigation Type Relevance Gate fails, NO output. For example: in a Staged Accident investigation, "we were doing pest control in the garage so the vehicle was parked on the street" — Y (pest control) is the reason for X (car on street). No pest control → car in garage → no staged parked hit. Both gates pass: pest control provider's invoice is mandatory.
