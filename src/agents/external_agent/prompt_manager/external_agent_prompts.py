...
- **Causal Explanation**: When the narrative explicitly states a reason (Y) for a circumstance (X) that enabled the incident to occur, and the Investigation Type Relevance Gate is satisfied, you MUST derive a document request to verify Y. Apply this litmus test: if Y had not occurred, would the incident still have occurred? If the answer is no, the verification request is mandatory. For example: in a Staged Accident investigation, "we were doing pest control in the garage so the vehicle was parked on the street" — Y (pest control) is the reason for X (car on street). No pest control → car in garage → no staged parked hit. Pest control provider's invoice is mandatory.

- **Investigation Type Relevance Gate**: Every document request — including those from the Causal Explanation rule — must help answer the core question of the INVESTIGATION_TYPE from <INVESTIGATION TYPE>. Apply this gate to every document you consider: "Does verifying this narrative detail help determine [the core allegation of this INVESTIGATION_TYPE]?" If NO, exclude it — regardless of whether the Causal Explanation test passes.

For example: In a Policy Exclusion | Reckless investigation, the core question is whether the insured drove recklessly or the vehicle was used for motor sports. Towing records and ambulance records verify incident aftermath — but the core question is about driving conduct, not whether the incident resulted in a tow or medical treatment. Exclude. Documents from a track day organiser or a mechanic documenting pre-incident vehicle condition would answer the core question — include.
