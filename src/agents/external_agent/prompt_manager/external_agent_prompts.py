...
- **No Duplicates**: Check each derived document type against the METHODOLOGY_DOCS below. If the narrative-derived document would request the same class of records as any methodology entry — even if the stated investigative purpose differs — skip it. Compare substance (what records are being requested) not purpose (why they are being requested). Example: if methodology requests "correspondence confirming the event's date and time" and the narrative derives "text messages/emails relating to notification of the incident" — both are the same class of records (personal communications from the policyholder around the incident period) even though the stated purposes differ → SKIP. Another example: scene photographs requested by any methodology entry already cover all photographs from the incident regardless of who captured them or what specific details they depict — do not create a separate entry for claimant-taken photographs. Only create a NEW entry if the records being requested are materially different from what the methodology entries already obtain.
...
5. **No Duplicates gate (MUST-run)**: For each derived document type, check all CRITICAL RULES. Check it against the METHODOLOGY_DOCS list — compare BOTH the doc_type AND the doc_details of each methodology entry. If the narrative-derived document would request the same class of records as any methodology entry (even if the stated investigative purpose differs), skip it. Match on substance, not label: correspondence/emails/texts are the same record class regardless of whether the methodology calls them "correspondence confirming the event's date and time" and the narrative describes "text messages/emails relating to notification of the incident." If a type is not a duplicate: it is a concrete, obtainable document from the claimant's own narrative → create a NEW standalone entry with its own distinct doc_type and concise 1-3 line doc_details (Comprehensive Drafting rule). If it is vague, a narrative statement from an involved party, or from a non-narrative source, exclude it. If you have identified more than 2 document types, keep only the 1-2 most critical. Causal Explanation entries (Step 4) take priority within this cap.
</TASK>
