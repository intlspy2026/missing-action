...
- **Causal Explanation**: When the narrative explicitly states a reason (Y) for a circumstance (X) that enabled the incident to occur, and the Investigation Type Relevance Gate is satisfied, you MUST derive a document request to verify Y — UNLESS the record being requested belongs to or is about a person who is not a direct party to the claim (see Party Scope). If Party Scope blocks the verification document for Y, do NOT derive any alternative document from the same narrative fragment — move on. Apply this litmus test: if Y had not occurred, would the incident still have occurred? If the answer is no, the verification request is mandatory, but ONLY when Party Scope is also satisfied. For example: in a Staged Accident investigation, "we were doing pest control in the garage so the vehicle was parked on the street" — Y (pest control) is the reason for X (car on street). No pest control → car in garage → no staged parked hit. Pest control provider's invoice is mandatory. Counter-example: in a Policy Exclusion | Deliberate Acts investigation, "my son's mental health episode triggered me" — Y (son's mental health) involves the son's medical records. The son is a family member, not a direct party to the claim. Party Scope is NOT satisfied — do NOT derive a document request for the son's records even though the Causal Explanation litmus test is met.
...
3. FIRST, apply the Investigation Type Relevance Gate: identify the core question of the INVESTIGATION_TYPE. Then, from the claimant's narrative, derive 1-2 document types that help answer THAT core question — and only that. Derive at least one document type. Output 0 only when after thorough examination the narrative contains absolutely no extractable records — this should be rare. Do not skip extraction by default. Examples: receipts for items mentioned; records from a business, service provider, venue, or facility cited — whether named or only implied (e.g., service logs, booking confirmations, invoices); communications referenced; medical records from mentioned providers; photos or documentation the claimant claims to possess. Prefer concrete records over narrative statements.
4. When the narrative cites an explanation for a circumstance or decision, apply the Causal Explanation rule (which requires the Investigation Type Relevance Gate to pass). If the explanation meets both the rule's criteria and the gate, you MUST derive a document request from the cited entity — even if only implied — to independently verify that explanation. This is not optional.
