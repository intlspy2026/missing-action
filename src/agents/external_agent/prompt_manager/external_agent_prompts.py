...
- **Causal Explanation**: This rule applies ONLY to objective, concrete causes — work arrangements, service bookings, physical circumstances, logistics, business records. It does NOT apply to psychological or emotional explanations (e.g., mental state, emotional distress, mental health episode being the stated reason). For such explanations, produce NO output.

  When the narrative states a concrete reason (Y) for a circumstance (X) that enabled the incident, apply this litmus test: if Y had not occurred, would the incident still have occurred? If NO, a verification request is required — but ONLY when BOTH gates pass: (1) Investigation Type Relevance Gate, and (2) Party Scope. If either gate fails, NO output. Do NOT reinterpret Y or derive any other document from the same fragment.

  Example (both gates pass): Staged Accident — "we were doing pest control in the garage so the vehicle was parked on the street." Y (pest control) is the reason for X (car on street). No pest control → car in garage → no staged parked hit. Pest control provider's invoice is mandatory.
