...
- TIMEFRAME X-patterns only: Follow the "Quantifiable timeframe only" and "No timeframe specified" rules. Do not substitute a vague relative description for a timeframe X-pattern — either compute exact dates from a quantifiable source or remove the timeframe clause entirely.
- For ENTITY and ANCHOR X-patterns: If you cannot determine an exact value, substitute with the most reasonable relative description grounded in the PREVIOUS VERSION doc_details and the case context. Leaving a non-timeframe X-pattern unfilled in the output is a critical error.

**Quantifiable timeframe only**: Compute EXACT date ranges ONLY when PREVIOUS VERSION doc_details specifies a concrete, quantifiable duration — a number + unit (e.g., "1 week prior to and after", "3 months surrounding", "2 days before"). Vague references like "close proximity", "surrounding the loss", "around the time of", and "close to the date of loss" are NOT quantifiable. Treat them as no timeframe specified. When PREVIOUS VERSION doc_details provides a quantifiable scope, the output MUST use that scope — not a wider default from the gold standard. The gold standard provides the request structure and language; the methodology provides the scope and duration.

**No timeframe specified**: When PREVIOUS VERSION doc_details provides no quantifiable timeframe — only vague references or no period at all — do NOT inject a computed timeframe from the gold standard. Remove the timeframe clause (e.g., "for the period X to Y", "for the period XX to XX") from the gold standard wording entirely. When the methodology provides no quantifiable scope, the output must not invent one.
