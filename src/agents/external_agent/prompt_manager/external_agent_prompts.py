...
a. Conditional qualifier: If INVESTIGATION PROCESSES includes a conditional qualifier (e.g., "if applicable", "if requested"), apply that condition against INITIAL REVIEW and ADDITIONAL INFORMATION. If the condition is not met — exclude. This rule does not apply to police enquiries — see RULE 1g.
...
g. Police attendance status — not the mere presence of the word "police" — determines whether a police enquiry is required. Police enquiries are governed exclusively by this rule — RULE 1a through RULE 1f and RULE 1k do not apply. A police enquiry passes the relevance filter if and only if it qualifies under one of the INCLUDE paths below. A loose mention of "police" in any subsection, or a "Police/..." heading with nil content, is NOT positive evidence of involvement. Apply the attendance-status test:
...
- INCLUDE — prior documented police interaction: INITIAL REVIEW documents a specific, named police action involving the insured vehicle or a direct party (e.g., plates seized/detained by a named police station, vehicle held in police custody, a police report or incident number for a prior claim involving the same vehicle, charges laid by a named station), AND that interaction is relevant to the current claim (e.g., same vehicle, vehicle condition at issue, same insured/driver). Scope the enquiry to obtaining information from that prior police interaction relevant to the current claim — not re-investigating the prior claim. A named police station performing a documented action is positive evidence of involvement, not a loose mention.
- EXCLUDE — attendance confirmed nil: police (or any station/source contacted) confirmed there was no attendance, no record, or no incident reported. If ANY source confirmed nil, EXCLUDE even if other stations could not be reached.
...
k. Default exclude: If no specific sub-rule above applies and the enquiry still has no anchored basis in INITIAL REVIEW or ADDITIONAL INFORMATION = exclude by default.
...
**RULE 2 - PARTY SCOPE**: Only frame enquiries around parties directly involved in the current claim under investigation. Use INITIAL REVIEW and ADDITIONAL INFORMATION to identify who the direct parties are. Individuals mentioned in prior claims, historical associations, or background checks within INITIAL REVIEW or ADDITIONAL INFORMATION are NOT direct parties to the current claim. Do not generate enquiries focused on associated individuals who are not direct parties. Law enforcement agencies (police) are not "individuals" for the purpose of this rule. Contacting a named police station documented in INITIAL REVIEW as having performed a specific action involving the insured vehicle or a direct party is a field enquiry — not investigating a party from a prior claim.
