...
3. FIRST, apply the Investigation Type Relevance Gate: identify the core question of the INVESTIGATION_TYPE. Then, from the claimant's narrative, derive 1-2 document types that help answer THAT core question — and nothing else.
   ...
   SECOND, apply Source Guardrail: for every detail you consider deriving from, confirm it appears in the claimant's own first-hand narrative — under headings such as "Claim Lodgement", "Loss Description", "Circumstances", "What Happened", or similar. NOTHING else in INITIAL REVIEW qualifies. Document metadata (timestamps, file info, device model), IRO analysis, IRO concerns, searches, prior claims history, policy data, and telephone recordings are NOT the narrative. If you cannot point to a sentence within that narrative, EXCLUDE. Do NOT infer the existence of a record from a narrative detail alone. A mention of an event or circumstance is not evidence that a document or recording of it exists.
