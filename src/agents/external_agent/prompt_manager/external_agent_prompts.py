...
**RULE 3 - RELEVANCE FILTER (INCLUDE-ONLY)**: You START from the position that EVERY document type from INVESTIGATION PROCESSES is EXCLUDED. A document type is INCLUDED ONLY when it is either (i) core to the investigation type — the allegation itself creates a need for it — or (ii) supported by a SPECIFIC FACT in the content of INITIAL REVIEW or ADDITIONAL INFORMATION (including search/background-check sections such as Caspar, RP Data, social media, and similar database outputs). Determine the current investigation type from INVESTIGATION TYPE in the context below.
    ...
    5. Tow truck / towing receipts: INCLUDE ONLY if the case explicitly involves a vehicle being towed. Exclude for property claims or fixed-location incidents where no towing is mentioned. ALSO EXCLUDE when the tow was organized or arranged by the insurer (Suncorp) — the towing records are already held internally and the external investigator does not need to request them. Treat the tow as insurer-organized when INITIAL REVIEW or ADDITIONAL INFORMATION:
        - states the tow was arranged/organized/sent by Suncorp, the insurer, or the assessor; OR
        - records a tow request/arrangement with NO sender attributed (e.g., "tow request sent", "tow arranged") — default these to insurer-organized; OR
        - records a tow request or tow occurring AFTER the claim lodgement date (Notice Date) — ONCE A TOW DATE IS MENTIONED in INITIAL REVIEW or ADDITIONAL INFORMATION, compare it against the Notice Date stated in INITIAL REVIEW; if the tow date is strictly later than the Notice Date, EXCLUDE (the vehicle is under the insurer's control post-lodgement and the tow is insurer-arranged). If no tow date is mentioned, do NOT apply this comparison — fall back to the sender-attribution rules above. If the tow date equals the Notice Date, do NOT apply this rule — fall back to the sender-attribution rules above; OR
        - records the vehicle at a Suncorp shed/yard/storage facility (e.g., "IV at Suncorp Shed", "arrival of IV at [address] Suncorp shed").
...
**RULE 1 - RELEVANCE FILTER**: For each enquiry from INVESTIGATION PROCESSES, assess whether it is applicable based on the facts in INITIAL REVIEW and ADDITIONAL INFORMATION. An enquiry passes the filter only when it satisfies ALL of the conditions below:
    ...
    b. Basis, named target & role-filler: If an enquiry references a scenario, person, or event that has no basis in INITIAL REVIEW or ADDITIONAL INFORMATION → exclude. Exclude any enquiry whose target (person, business, or organization) is not named in INITIAL REVIEW or ADDITIONAL INFORMATION. If an INVESTIGATION PROCESSES template references a role but no specific party filling that role appears in INITIAL REVIEW or ADDITIONAL INFORMATION → exclude that enquiry entirely. Never assume an entity exists because a claim detail (damage, estimate, repair need, loss amount, vehicle condition) implies one might — a template passes the filter only when INITIAL REVIEW or ADDITIONAL INFORMATION reference a specific entity — named or described as existing — filling that role.

    c. Repairer / panel shop / vehicle assessor: If no repairer, panel shop, smash repairer, or vehicle assessor is named in INITIAL REVIEW or ADDITIONAL INFORMATION, a repairer does not exist → exclude any template referencing a repairer.

    d. Police attendance status — not the mere presence of the word "police" — determines whether a police enquiry is required. INCLUDE a police enquiry when attendance is confirmed (police attended, a police report was issued/numbered, charges laid, brief of evidence, or matter before court) OR when attendance is unconfirmed but under active enquiry (the IRO called/contacted a station but could not yet reach them, is awaiting a response, or it is recorded as unknown/unsure whether police attended) AND no source has confirmed nil. EXCLUDE when any station/source confirmed there was no attendance, no record, or no incident (even if other stations could not be reached), or when police attendance is not mentioned at all or appears only as a nil/no/"No details"/"Nil results" mention with no record of any police enquiry made or attempted. A loose mention of "police" in any subsection, or a "Police/..." heading with nil content, is NOT positive evidence of involvement.

    e. Tow records and tow-operator enquiries: EXCLUDE when the tow was organized by the insurer (Suncorp) — the records are held internally. Treat the tow as insurer-organized when the notes state the tow was arranged/sent by Suncorp/the insurer/assessor, record a tow request with no sender attributed, OR record the vehicle at a Suncorp shed/yard/storage facility. Separately, IF a tow date is mentioned in INITIAL REVIEW or ADDITIONAL INFORMATION: compare it against the Notice Date (claim lodgement date) stated in INITIAL REVIEW — if the tow date is strictly later than the Notice Date, EXCLUDE (the vehicle is under the insurer's control post-lodgement and the tow is insurer-arranged). If the tow date equals the Notice Date, do NOT apply this comparison — fall back to the sender-attribution signals above. If no tow date is mentioned, do NOT apply the post-lodgement comparison — fall back to the sender-attribution signals. Only INCLUDE a tow enquiry when the insured or a third party organized the tow independently AND the tow occurred on or before the claim lodgement date (Notice Date).

    f. Default exclude: If no specific sub-rule above applies and the enquiry still has no anchored basis in INITIAL REVIEW or ADDITIONAL INFORMATION → exclude by default.
