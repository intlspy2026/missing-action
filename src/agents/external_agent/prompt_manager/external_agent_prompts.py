...
- TIMEFRAME (quantifiable): When PREVIOUS VERSION doc_details specifies a concrete, quantifiable duration (a number + unit, e.g., "1 week", "3 months", "2 days"), compute an EXACT date range anchored to the date of loss from Step 2, output in format "[DD Month YYYY] to [DD Month YYYY]". Example: "1 week prior to and after the incident on 21 November 2025" -> "14 November 2025 to 28 November 2025". Do NOT output relative phrasings — convert to absolute dates.
- TIMEFRAME (not quantifiable): Follow the "No timeframe specified" rule below — remove the timeframe clause from the gold standard entirely.
- ENTITY (name, identifier, location, address): replace with the specific value from the case context.
- ANCHOR (incident date, policy date): replace with the long-form date (DD Month YYYY) from Step 2.
...
**No timeframe specified**: When PREVIOUS VERSION doc_details provides no quantifiable timeframe — only vague references or no period at all — do NOT inject any dates from the gold standard. Remove both the timeframe range clause (e.g., "for the period XX to XX") AND any single-date references (e.g., "For the dates XXXX") from the gold standard wording entirely. When the methodology provides no quantifiable scope, the output must not invent one.
