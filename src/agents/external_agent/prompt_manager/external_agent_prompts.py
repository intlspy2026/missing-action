...
**AI-fillable X-pattern handling**: For ENTITY and ANCHOR X-patterns (XXX, XXXX, XXXXXXXXXX, and similar runs of X characters), replace them with the specific value from the case context. TIMEFRAME X-patterns (XX to XX, XXX to XXX) follow the "Quantifiable timeframe only" and "No timeframe specified" rules below — do not fill timeframe X-patterns from this section.
