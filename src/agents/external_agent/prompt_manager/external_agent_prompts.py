...
g. Police attendance status — not the mere presence of the word "police" — determines whether a police enquiry is required. A loose mention of "police" in any subsection, or a "Police/..." heading with nil content, is NOT positive evidence of involvement. Apply the attendance-status test:

- INCLUDE — attendance confirmed: police attended the incident, a police report was issued or a police incident/reference number is recorded, charges laid, a brief of evidence exists, or the matter is before court.
- INCLUDE — attendance unconfirmed but under active enquiry: the IRO made or attempted police enquiries (e.g., called or contacted a police station) but could not yet reach them, is awaiting a response/callback, or it is recorded as unclear/unknown/unsure whether police attended — AND no station or source has confirmed there was no attendance/record/incident. An open or pending enquiry is sufficient to include only when nil has not already been confirmed.
- EXCLUDE — attendance confirmed nil: police (or any station/source contacted) confirmed there was no attendance, no record, or no incident reported. If ANY source confirmed nil, EXCLUDE even if other stations could not be reached.
- EXCLUDE — not mentioned / nil: police attendance is not mentioned at all, or the only reference is a nil/no/"No details"/"Nil results" mention with no record anywhere of any police enquiry having been made or attempted.
...
i. Tenancy/rental status: If a methodology enquiry references tenancy, rental, lease, landlord, or tenant — only INCLUDE when INITIAL REVIEW or ADDITIONAL INFORMATION explicitly references a tenancy arrangement (e.g., "tenants", "rental agreement", "lease", "property manager", "rental payments", "tenanted"). Family members or relatives living in or staying at the property — even when the insured does not reside there — do NOT constitute a tenancy. EXCLUDE when the sole basis is that the insured lives at a different address or that the property is occupied by family members. Also exclude for owner-occupiers and when tenancy/rental status is not mentioned or cannot be determined from the case facts.
j. Employer/workplace verification: If a methodology enquiry references verifying employment, employer details, work attendance, workplace records, or contacting an employer — only INCLUDE when INITIAL REVIEW or ADDITIONAL INFORMATION shows the insured was at work, travelling to/from work, or the incident occurred during work hours or at a workplace. EXCLUDE when the incident has no connection to the insured's employment or workplace.
k. Service/Repairer of the vehicle: EXCLUDE by default. INCLUDE ONLY if INITIAL REVIEW or ADDITIONAL INFORMATION explicitly references a service having been performed on the subject vehicle (e.g., "vehicle was serviced", "repairs were carried out", "service history available") AND identifies the service provider by name. A mention of the vehicle being assessed, quoted, inspected, or needing repairs does not satisfy this condition — there must be an explicit indication that a service or repair was actually performed. If the service provider is not named, exclude even if vehicle service is mentioned.
l. Neighbour enquiries for Property investigation types involving internal building conditions (Escape of Liquid, Condition of Property, and similar property-internal-defect types): If a methodology enquiry references neighbours, adjacent properties, shared walls, external pipes, or environmental factors contributing to the condition — EXCLUDE when the subject property is a standalone dwelling (house, detached dwelling with no shared walls). INCLUDE ONLY when the property address indicates shared walls with adjacent properties (unit, apartment, flat, townhouse, duplex, semi-detached, or similar attached-dwelling descriptor). Determine shared-wall status from the address: a standalone street number and street name without unit/apt/flat qualifiers indicates a standalone dwelling; "Unit X/X", "Apt X", "Flat X", or "X/X" address patterns indicate shared walls. Standalone houses are always excluded because neighbours cannot provide relevant information about internal building conditions (plumbing, water escape, structural defects) within a detached dwelling.
m. Default exclude: If no specific sub-rule above applies and the enquiry still has no anchored basis in INITIAL REVIEW or ADDITIONAL INFORMATION = exclude by default.
...
**RULE 9 - CROSS-RESPONDENT MERGE (applies after RULE 8, before STYLE)**: When methodology entries target different respondent categories at the same location, any combination of corroborating witness interview, CCTV footage request, and area canvass that serve the same investigative purpose MUST be merged into a single output enquiry combining all sub-tasks into enquiry_detail. Location and purpose together determine the merge boundary — different respondent categories, people, or timeframes at the same location do not prevent merging when the purpose is the same. For example:

- Pre-incident purpose: a corroborating witness interview about the insured's activities at a venue before the incident, a CCTV request from that venue for the pre-incident period, and an area canvass of that venue — merge into one pre-incident venue enquiry.
- Incident-scene purpose: a corroborating witness interview about the insured at the loss location, incident-scene CCTV from nearby premises, and an area canvass of those premises — merge into one incident-scene enquiry.

After merging, perform a final location review: if any merged entry combines sub-tasks from genuinely different locations (e.g., a pre-incident venue and the incident-scene location), split it back into separate entries by location. Entries at genuinely different locations must remain separate.
...
**RULE 5 - NARRATIVE GUARDRAILS**: When deriving enquiries from the claimant's incident account:
    ...
    e. Image and photograph requests must target independent third-party sources (witnesses, nearby businesses, CCTV operators, independent record holders), not the insured, additional insured, or driver. When the narrative describes a scene, location, or event where photographic evidence may exist from independent sources, derive an enquiry targeting those sources. Do not derive image requests from the insured, additional insured, or driver. Image and photograph requests do not require a standalone enquiry — merge them into the area canvass enquiry at the relevant location.
...
**RULE 6 - AGGREGATE BY THEME**: You receive methodology-driven enquiries listed flatly. Many belong to a smaller number of underlying themes. You MUST aggregate all enquiries (methodology + narrative-derived) that belong to the same theme into a single output enquiry — combining their sub-tasks, sub-questions and party-specific variations into enquiry_detail. Do NOT emit one output enquiry per source bullet, per sub-task or per party. Output MUST be one enquiry per theme you identify, not one per source line. If a narrative-derived enquiry overlaps with a methodology-derived enquiry, merge them into one.
    ...
    **Same-theme merge tests** — apply these before finalising. If any test matches, the enquiries MUST be one, not several:
    ...
    e. **Same purpose + location merge**: any combination of corroborating witness interview, CCTV footage request, and area canvass at the same location that serve the same investigative purpose MUST be merged into a single enquiry combining all sub-tasks into enquiry_detail. Different people or timeframes at the same location do not prevent merging when the purpose is the same.
...
3. **Possessive Form Rules — Individual (non-business) insured type**:
   - **SINGLE insured assigned, NO other parties**: Keep "your" as-is. Do NOT insert any name.
     ...
     Given insured=Anjali, original text "A copy of documents issued by the Police including but not limited to charge sheet, brief of evidence, court appearance dates and any other document relevant to this matter.":
     CORRECT: "A copy of your documents issued by the Police including but not limited to charge sheet, brief of evidence, court appearance dates and any other document relevant to this matter." — "your" inserted, insured's name MUST NOT appear.
     WRONG: "A copy of Anjali's documents issued by the Police including but not limited to charge sheet, brief of evidence, court appearance dates and any other document relevant to this matter." — insured's name "Anjali" must NOT appear in possessive form; use "your" instead.
     Given insured=Anjali, original text "Please sign and return the authority for Police, Medical, Hospital Records (attached).":
     CORRECT: "Please sign and return your authority for Police, Medical, Hospital Records (attached)." — "the" dropped per Rule 7, "your" inserted.
     WRONG: "Please sign and return Anjali's authority for Police, Medical, Hospital Records (attached)." — insured's name "Anjali" must NOT appear.
   ...
   - **Insured + other parties (e.g. driver, witness)**: Keep "your" for the insured. Insert "'s" for all other party names. NEVER replace "your" with the insured's actual name.
     ...
     If the text contains no "your"/"you", insert "your and [OtherParty]'s" (or "your, [Party1]'s, and [Party2]'s" for 3+ parties with Oxford comma) before the document noun (applying Rule 7 Determiner Absorption).
     Given insured=Anjali, driver=Bob, original text "A copy of documents issued by the Police including but not limited to charge sheet, brief of evidence, court appearance dates and any other document relevant to this matter.":
     CORRECT: "A copy of your and Bob's documents issued by the Police including but not limited to charge sheet, brief of evidence, court appearance dates and any other document relevant to this matter." — "your" for insured, "Bob's" for driver.
     WRONG: "A copy of Anjali's and Bob's documents issued by the Police including but not limited to charge sheet, brief of evidence, court appearance dates and any other document relevant to this matter." — insured name "Anjali" must NOT replace "your".
     EXCEPTION (no pronoun, multiple insureds flag): Insert "[Name]'s and [OtherParty]'s" without "your".
     EXCEPTION: If the context states "Multiple insureds in policy: true", the insured MUST be replaced with their possessive name form ("[Name]'s") — never use "your". This is because "your" is ambiguous when the policy has multiple insureds.
...
5. VERIFY: Before returning, check:
   ...
   - SINGLE INSURED NO-PRONOUN VERIFY: If single insured, no other parties, individual type, no multiple insureds flag, and the original text has no "your"/"you": "your" MUST appear before the document noun. If the insured's possessive name appears there instead, this is WRONG — replace with "your" now. The insured's name must NOT appear in the output at all for this scenario.
