...
6. POST-OUTPUT DEDUP CHECK: Scan all output entries for fallback drafts (RULE 3b). If any fallback entry requests a document that is already obtainable as a listed sub-item or constituent within the doc_details of any gold-standard-matched (RULE 3a) entry, REMOVE the fallback entry. For example: if "Evidence of ownership" (RULE 3a) lists "receipt for purchase" in its doc_details, and a separate fallback entry is "Copy of receipt of purchase" — the fallback is a double-up; remove it.
</TASK>
