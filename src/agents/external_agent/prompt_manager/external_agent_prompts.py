...
**RULE 3 - RELEVANCE FILTER (INCLUDE-ONLY)**: You START from the position that EVERY document type from INVESTIGATION PROCESSES is EXCLUDED. A document type is INCLUDED ONLY when it is either (i) core to the investigation type — the allegation itself creates a need for it — or (ii) supported by a SPECIFIC FACT in the content of INITIAL REVIEW or ADDITIONAL INFORMATION (including search/background-check sections such as Caspar, RP Data, social media, and similar database outputs). Determine the current investigation type from INVESTIGATION TYPE in the context below.
    ...
    5. Tow truck / towing receipts: INCLUDE ONLY if the case explicitly involves a vehicle being towed. Exclude for property claims or fixed-location incidents where no towing is mentioned. ALSO EXCLUDE when the tow was organized or arranged by the insurer (Suncorp) — the towing records are already held internally and the external investigator does not need to request them. Treat the tow as insurer-organized when INITIAL REVIEW or ADDITIONAL INFORMATION:
        ...
        A tow destination at a salvage/auto-auction provider does NOT by itself make the tow insurer-organized — it still requires the sender to be the insurer or unattributed per above. Only INCLUDE when the insured or a third party organized the tow independently of the insurer.
    ...
    15. Police Documents: Base the decision on the police attendance status recorded across INITIAL REVIEW and ADDITIONAL INFORMATION — a loose mention of "police" may appear in any subsection (a searches/enquiries section, the narrative, Independent Verification notes, concerns, etc.), so assess the whole notes, not just one heading. The mere presence of the word "police" does NOT establish police involvement. Apply the attendance-status test IN THIS ORDER — stop at the first step that matches:
        - STEP 1 — EXCLUDE if any nil confirmed: If ANY police force, station, or source contacted in INITIAL REVIEW or ADDITIONAL INFORMATION confirmed there was no attendance, no record, or no incident reported (e.g., "advised no record or incidents reported", "police confirmed they did not attend", "Police/Citec: Nil results"), EXCLUDE immediately. This is dominant — once any source confirmed nil, do NOT include even if other stations could not be reached or the IRO was instructed to check for police attendance. A triage note such as "suggest checking for police attendance" or "IRO to contact" is a recommendation to make enquiries, NOT a nil result; if the enquiry was then made and returned nil, STEP 1 applies and you EXCLUDE.
        - STEP 2 — INCLUDE if attendance confirmed: police attended the incident, a police report was issued or a police incident/reference number is recorded, charges were laid, a brief of evidence exists, or the matter is before court.
        - STEP 3 — INCLUDE if attendance unconfirmed but under active enquiry: the IRO made or attempted police enquiries (e.g., called or contacted a police station) but could not yet reach them, is awaiting a response/callback, or it is explicitly recorded as unclear/unknown/unsure whether police attended — AND STEP 1 did not fire (no source has confirmed nil). An open or pending enquiry is sufficient to include only when no nil has been confirmed.
        - STEP 4 — EXCLUDE if not mentioned / nil: police attendance is not mentioned at all, or the only reference is a nil/no/none/"No details"/"Nil results" mention (in a Police section or loose in any subsection) with no record anywhere of any police enquiry having been made or attempted.
