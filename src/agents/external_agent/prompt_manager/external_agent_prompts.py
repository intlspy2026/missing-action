...
3. FIRST, apply the Investigation Type Relevance Gate: identify the core question of the INVESTIGATION_TYPE. Then, from the claimant's narrative, derive 0-2 document types that help answer THAT core question — and only that. Check each derived type against Party Scope: the records must belong to and be about a direct party. If Party Scope blocks the record, do NOT derive it. Examples: receipts for items mentioned; records from a business, service provider, venue, or facility cited (e.g., service logs, booking confirmations, invoices); communications referenced; photos the claimant claims to possess. Prefer concrete records over narrative statements.
