...
def get_graph(llm: BaseChatModel) -> StateGraph:
    ...
    def _classify_hitl(
        ...
        - "feedback": user gives feedback that mentions which section, concern, document, enquiry, or item the change applies to — even loosely (e.g. "change John to Jerry in phone records" names a section, so apply it everywhere within that section). If the Current review section provides context, classify as feedback when the command creates a new item in the section (e.g. "add enquiry: check X with Y") or uniquely identifies the target within the section — by ordinal (e.g. "remove the second document"), by description (e.g. "update the enquiry about neighbour interviews"), or by name. A command that has a clear, singular target IS feedback.
        - "ambiguous": user gives a generic edit command with no uniquely locatable target — even with section context, it could apply to multiple items. Examples: "change date to 25 May" (which enquiry?), "update the address" (which item?), "fix the spelling" (which entry?). Also mark ambiguous when there is no Current review section context at all. However, if the user explicitly scopes the command to apply across all matching items (e.g. "change date to 25 May — apply all", "update the address everywhere"), classify as feedback — the user has consciously accepted multi-item scope. In this case, write a clarifying question in task_summary asking the user which specific item they want updated.
            ...
            if incoming_artifact and intent in ("accept", "feedback", "preview_update"):
                try:
                    ...
                    parsed_list = (
                        ...
                        getattr(parsed, "enquiries_set", None)
                    )
    ...
    def _next_section(current: str, selected_sections: list[str]) -> str:
        ...
        Order: key_concerns → doc_request → additional_enquiries → assemble_plan
        """
        order = [
    ...
    def initialise_query(state: "ExternalAgentState") -> Command:
        ...
        if decision and decision.intent == "accept" and hitl_artifact:
            ...
            insured_details = hitl_artifact.get("insured_details")
    ...
    def generate_key_concerns(state: "ExternalAgentState") -> Command:
        ...
        if feedback:
            ...
            prompt = KEY_CONCERNS_FEEDBACK_PROMPT.format(
    ...
    async def generate_doc_request_async(state: "ExternalAgentState") -> Command:
        ...
        knowledge_json = None
        insured_type_label = (
            "business" if (state.get("insured_type") or "").strip().lower() == "yes"
            else "individual"
        )
        ...
        if feedback:
            ...
            prompt = DOC_REQUEST_FEEDBACK_PROMPT.format(
    ...
    async def generate_enquiries_async(state: "ExternalAgentState") -> Command:
        ...
        if feedback:
            ...
            prompt = ADDITIONAL_ENQUIRIES_FEEDBACK_PROMPT.format(
