...
def get_graph(llm: BaseChatModel) -> StateGraph:
    ...
    def _classify_hitl(
        ...
        - "ambiguous": user gives a generic edit command with no uniquely locatable target — even with section context, it could apply to multiple items. Examples: "change date to 25 May" (which enquiry?), "update the address" (which item?), "fix the spelling" (which entry?). Also mark ambiguous when there is no Current review section context at all. In this case, write a clarifying question in task_summary asking the user which specific item they want updated.
