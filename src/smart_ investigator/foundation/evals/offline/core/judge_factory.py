...
def create_output_only_judge(
    ...
    """
    Rubric-following output-only judge.

    Evaluates based on outputs only, without requiring inputs.
    Use for metrics like tone_compliance where only the response text matters.
    """
    return make_judge(
        name=name,
        instructions=(
            f"{guidelines}\n\n"
            f"Evaluate the {name} based on the response below:\n"
            "Response: {{outputs}}"
        ),
        feedback_value_type=feedback_value_type,
        model=model
