...
class TraceRetriever:
    ...
    def get_recent_sessions(
        ...
        # Extract session_id from request["request"]["user"] field
        def extract_session_id(request):
            ...
            if isinstance(request, dict):
                # Handle nested structure: request -> request -> user
                inner_request = request.get("request", request)
                return inner_request.get("user", "")
            return ""

        traces["session_id"] = traces["request"].apply(extract_session_id)

        for session_id, group in traces.groupby("session_id"):
            if session_id and len(group) >= min_traces:
                sessions[session_id] = group.sort_values("request_time")
